...
input.addEventListener(
  ...
  debounce(function () {
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      var mailCells = rows[i].getElementsByTagName("td")[1];
      var match = false;

      if (
        nameCells &&
        mailCells &&
        (nameCells.textContent.toLowerCase().indexOf(searchValue) != -1 ||
          mailCells.textContent.toLowerCase().indexOf(searchValue) != -1)
      ) {
        match = true;
      }
...
var mockData = [
  {
    name: "Wade Warren",
    mail: "wadewarren@gmail.com",
    role: "Admin",
  },
  {
    name: "Brooklyn Simmons",
    mail: "brooklynsimmons@gmail.com",
    role: "Accountant",
  },
];
...
getData()
  .then((data) => {
    ...
    data.forEach(function (rowData) {
      ...
      fileCell.innerHTML = rowData.name;
      adminCell.innerHTML = rowData.mail;
      statusCell.innerHTML = `<div class="relative">
                    <button class="
                    bg-[#747474] bg-opacity-10 text-[#6F6F6F]
                     p-2 w-28 h-7 dropdown rounded-md text-sm border dropdown-svg text-center flex justify-between items-center">
                    ${rowData.role}<svg class="w-4 h-4 ml-1 dropdown-svg" aria-hidden="true" fill="none" stroke="currentColor"
                    ...
                    <li>
                        <a href="#"
                            ...
                            <div class="bg-[#747474] mr-2  rounded-full w-3 h-3"></div>
                            <p>
                                Accountant
                            </p>
                        </a>
